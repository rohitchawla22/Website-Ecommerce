...
$query_sql1 = oci_parse( $connect1 , "SELECT * FROM user_signup GROUP BY City");
oci_execute(query_sql1);

$dyn_table1 ='<tr>';
While(($row = oci_fetch_assoc($sql1) != false ){
...
$query_sql3 = oci_parse( $connect1 , "SELECT * FROM user_signup");
oci_execute( $query_sql3);
$dyn_table3 ='<tr>';
While(($row = oci_fetch_assoc($sql3) != false){
...
<?php
					//	mysql_select_db($database_connect, $connect);
						$connect1 = oci_connect($username_connect, $password_connect, $database_connect);
						$sql2 = oci_parse( $connect1,"SELECT * FROM product ORDER BY qtysold DESC LIMIT 3");
						oci_execute($sql2);
						$dyn_table2 ='<tr>';
...
<title>Bazzinga | Sort User Db</title>
